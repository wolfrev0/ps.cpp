...
template<class T> struct LineLegacy{
	...
	LineLegacy(const Vec2<T>& s,const Vec2<T>& e):sv(s),ev(e){if(sv>ev)swap(sv,ev);}

	bool operator==(const LineLegacy& r)const{return tan()=r.tan()and s==r.s;}
...
template<class T> struct Segment:public LineLegacy<T>{
	...
	virtual bool valid_intersect(const Vec2<T>& p)const override{
		if(abs(sv.x-ev.x)<eps)
			return min(sv.y,ev.y)-eps<=p.y and p.y<=max(sv.y,ev.y)+eps;
		if(abs(sv.y-ev.y)<eps)
			return sv.x-eps<=p.x and p.x<=ev.x+eps;
		return is_valid(p);
	}
