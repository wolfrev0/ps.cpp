...
struct Flow : public Graph<FlowW> {
	...
	tuple<Arr<Edge>, Arr<int>, Arr<int>> cuts() {
		...
		sort(r.begin(), r.end(), [&](auto x, auto y) {
			return make_pair(x.s, x.e) < make_pair(y.s, y.e);
		});
		...
		Arr<int> srcv, snkv;
		for(int i = 0; i < n - 2; i++) (pvis[i] ? srcv : snkv).pushb(i);
...
private:
	...
	int _dinic(int flow) {
		...
		while(sz(q)) {
			...
			q.pop();
			for(auto i : g[x])
				if(i.w.cap and d[i.e] > d[i.s] + 1) d[i.e] = d[i.s] + 1, q.push(i.e);
		}
	...
	pair<i64, i64> process_mcmf(i64 flow) {
		Arr<FlowW> d;
		Arr<Edge> p;
		if(!spfa(d, p, src) || p[snk].s == -1) return {};
		for(auto& e : cons_path(d, p, snk)) flow = min(flow, g[e.s][e.ei].w.cap);
		i64 cost = 0;
