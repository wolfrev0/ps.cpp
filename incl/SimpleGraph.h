...
struct SimpleGraph : public DirGraph {
	...
	Arr<int> art_points() {
		...
		Arr<int> ord(n, -1), ret;
		for(int i = 0; i < n; i++)
			if(ord[i] == -1) dfs_ap(i, -1, ordi, ord, ret);
		return ret;
	}
	...
	Arr<pair<int, int>> bridges() {
		...
		for(int i = 0; i < n; i++)
			if(ord[i] == -1) dfs_bridge(i, -1, ordi, ord, ret);
		return ret;
	}
...
private:
	int dfs_ap(int v, int p, int& ordi, Arr<int>& ord, Arr<int>& ans) {
		...
		if(fail and sc >= 2) ans.pushb(v);
		return mo;
	}

	// check if v-p is bridge
	int dfs_bridge(int v, int p, int& ordi, Arr<int>& ord,
	               Arr<pair<int, int>>& ans) {
		int mo = ord[v] = ordi++;
		for(auto i : g[v])
			if(i.e != p)
				mo = min(
				    mo, ord[i.e] == -1 ? dfs_bridge(i.e, v, ordi, ord, ans) : ord[i.e]);
		if(p != -1 and mo == ord[v]) ans.pushb({v, p});
		return mo;
	}
};
