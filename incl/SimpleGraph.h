...
private:
	int dfs_ap(int v,int p,int& ordi,Arr<int>& ord,Arr<int>& ans){
		...
		for(auto i:adj[v]){
			...
			if(ord[edg[i].opp(v)]==-1){
				int res=dfs_ap(edg[i].opp(v),v,ordi,ord,ans);
				mo=min(mo,res);
				fail|=res>=ord[v];
				sc++;
	...
	int dfs_bridge(int v,int p,int& ordi,Arr<int>& ord,Arr<pair<int,int>>& ans){
		...
		for(auto i:adj[v])
			if(edg[i].opp(v)!=p)
				mo = min(mo, ord[edg[i].opp(v)]==-1 ? dfs_bridge(edg[i].opp(v),v,ordi,ord,ans) : ord[edg[i].opp(v)]);
		if(~p and mo==ord[v])ans.pushb({v,p});
		return mo;
	}
};
