...
template<class T> Arr<T> factorials(int n) {
	Arr<T> r(n, 1);
...
template<class T, int n, int k> Arr<Arr<T>> binoms() {
	...
	for(int i = 1; i < n; i++) {
		r[i][0] = 1;
		for(int j = 1; j < k; j++) r[i][j] = r[i - 1][j - 1] + r[i - 1][j];
	}
	return r;
}
//<Adjust Formaula>
...
// if (x,y) is one of root, (x+t*b/g, y-t*a/g) is also root.(t is arbitary
// integer) when x,y have boundary, see (jung2381187, koosaga)'s code at
// boj_11661, linearity? <Multivariable Linear Deophantine> ax+by+cz=k =>
// gcd(a,b)w+cz=k && ax+by=gcd(a,b)w (extend it recursively)
...
Arr<Arr<int>> sieve_divs(int n) {
	Arr<Arr<int>> r(n);
	for(int i = 1; i < n; i++) {
		for(int j = i; j < n; j += i) r[j].emplb(i);
	}
	return r;
}
