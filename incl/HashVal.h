...
struct HashVal {
	u64 h[2];
	HashVal(u64 h1 = 0, u64 h2 = 0, int cnt = 0) : cnt(cnt) {
		h[0] = h1;
		h[1] = h2;
	}

	void push_back(HashVal x) {
		for(int i = 0; i < 2; i++) h[i] *= p[i], h[i] += x.h[i];
		cnt++;
	}
	void push_front(HashVal x) {
		for(int i = 0; i < 2; i++) h[i] += x.h[i] * fp(p[i], cnt);
		cnt++;
	}
	void pop_back(HashVal x) {
		for(int i = 0; i < 2; i++) h[i] -= x.h[i], h[i] *= inv(p[i]);
		cnt--;
	}
	void pop_front(HashVal x) {
		for(int i = 0; i < 2; i++) h[i] -= x.h[i] * fp(p[i], cnt - 1);
		cnt--;
	}

	void insert(HashVal x) {
		for(int i = 0; i < 2; i++) h[i] += fp(p[i], x.h[i]);
		cnt++;
	}
...
private:
	...
	//지금은 cnt 길이가 가변이라 cxp효과를 못받지만, 직접 짜거나 개선하면
	//cxp효과로 log제거 가능하다.
