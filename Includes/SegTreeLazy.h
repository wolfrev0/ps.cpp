...
struct SegTreeLazy{
  ...
  T query(int s, int e, bool revord=false){return query(1,0,n,s,e,revord);}
private:
  const int n;
  vector<T> tree;
  vector<U> lazy;
  ...
  T query(int cur, int cs, int ce, int s, int e, bool revord){
    propagate(cur,cs,ce);
    if (s>=ce||e<=cs)
      return F::idT();
    if (s<=cs&&ce<=e)
      return tree[cur];
    if(revord)
      return F::q(query(cur*2+1,(cs+ce)/2,ce,s,e,revord),query(cur*2,cs,(cs+ce)/2,s,e,revord));
    return F::q(query(cur*2,cs,(cs+ce)/2,s,e,revord),query(cur*2+1,(cs+ce)/2,ce,s,e,revord));
  }
};