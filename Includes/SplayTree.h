...
template<typename T>
struct SplayTree{
  SplayTree(int n, T id_qry, T id_upd, const function<T(const T&, const T&)>& queryf, const function<T(const T&, const T&)>& updf)
    :n(n), id_qry(id_qry), id_upd(id_upd), queryf(queryf), updf(updf)
  {
    root=new_node();
	...
	SplayTree(int n=0)
		:SplayTree(n, T::zero(), T::zero(), [](const T& a, const T& b) {return a + b; }, [](const T& a, const T& b) {return a + b; })
  {}

  void update(int i, T val) {
    i++;//cuz left mock node
    auto x=interval(i, i+1);
    x->val=updf(x->val, val);
    renew(x);
  }
